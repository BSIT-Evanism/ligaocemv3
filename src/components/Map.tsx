...
import { MapContainer, TileLayer, Marker, Popup, useMapEvents, Polyline, Polygon, useMap, LayersControl } from "react-leaflet";
...
type MapProps = {
    ...
    onMapClickAction?: (coordinates: { latitude: number; longitude: number }) => void;
...
function configureDefaultMarker(): void {
    const defaultIconPrototype = Icon.Default.prototype as unknown as { _getIconUrl?: unknown };
...
function ClickToAddMarkers({ onAdd, enabled = true }: { onAdd: (latlng: LatLngTuple) => void; enabled?: boolean }) {
    useMapEvents({
...
function ClickToAddClusters({ onAdd, enabled = true }: { onAdd: (coordinates: { latitude: number; longitude: number }) => void; enabled?: boolean }) {
    useMapEvents({
        click(e: LeafletMouseEvent) {
            if (!enabled) return;
...
function ClickToAddPolyline({ onAdd, enabled = true }: { onAdd: (latlng: LatLngTuple) => void; enabled?: boolean }) {
    useMapEvents({
        click(e: LeafletMouseEvent) {
            if (!enabled) return;
            onAdd([e.latlng.lat, e.latlng.lng]);
...
export default function Map({ center = [14.5995, 120.9842], zoom = 12, className, markers, onMarkersChangeAction, enableAddMarkers = true, polygon, polygonColor = '#3b82f6', polyline, enableAddPolyline = false, onPolylineChangeAction, fitToGeometry = true, maxZoom = 18, enableLayerControls = true, clusters = [], enableAddClusters = false, onClusterClickAction, onMapClickAction, clearTempMarker = false }: MapProps) {
    const { markers: storeMarkers, addMarker } = useMapStore();
    const [tempClusterMarker, setTempClusterMarker] = useState<LatLngTuple | null>(null);

    const getAssetUrl = (val: unknown): string => {
        if (typeof val === 'string') return val;
        if (val && typeof val === 'object' && 'src' in (val as Record<string, unknown>)) {
            return (val as { src: string }).src;
        }
        return '';
    };

    const defaultIcon = useMemo(() => new Icon({
    ...
    const [localMarkers, setLocalMarkers] = useState<LatLngTuple[]>(() => markers ?? (center ? [center] : []));
    const [localPolyline, setLocalPolyline] = useState<LatLngTuple[]>(() => polyline ?? []);
    ...
    function FitToGeometry({ enabled = true }: { enabled?: boolean }) {
        ...
        useEffect(() => {
            ...
            const bounds = new LatLngBounds([firstLat, firstLng], [firstLat, firstLng]);
    ...
    return (
        <MapContainer center={center} zoom={zoom} maxZoom={maxZoom} minZoom={1} zoomSnap={0.25} className={`${className ?? "h-[400px] w-full rounded-md"} leaflet-map-container`}>
            {enableLayerControls ? (
            ...
            <ClickToAddMarkers onAdd={handleAdd} enabled={enableAddMarkers && !enableAddClusters} />
            <ClickToAddPolyline onAdd={handleAddPolyline} enabled={enableAddPolyline} />
            <ClickToAddClusters onAdd={handleMapClick} enabled={enableAddClusters} />
            <FitToGeometry enabled={fitToGeometry} />
            {!enableAddClusters && storeMarkers.map((pos, idx) => (
                <Marker key={`${pos[0]}-${pos[1]}-${idx}`} position={pos} icon={defaultIcon}>
                    <Popup>
            ...
            {enableAddClusters && tempClusterMarker && (
                <Marker position={tempClusterMarker} icon={defaultIcon}>
                    <Popup>
                        Selected location: {tempClusterMarker[0].toFixed(6)}, {tempClusterMarker[1].toFixed(6)}
                    </Popup>
                </Marker>
            )}
            {clusters.filter(cluster => cluster.coordinates).map((cluster) => (
                <ClusterMarker
                    key={cluster.id}
            ...
            {localPolyline.length > 1 && (
                <Polyline positions={localPolyline} pathOptions={{ color: '#10b981', weight: 3 }} />
            )}
            {polygon && polygon.length > 2 && (
                <Polygon positions={polygon} pathOptions={{ color: polygonColor, weight: 2, fillOpacity: 0.2 }} />
            )}
        </MapContainer>
    );
}
